...
/***** We replace the following for use in Karger's mincut alg
...
#define DOUBLE weight_t

typedef node dyn_item;

/* some extra functions for memory management */
void dyn_init(int n);   /* preallocate memory to allow an n-node tree.
				 must call this first */

void dyn_reset();             /* destroy all trees, but keep memory so we
				 can start again (restores to same state as 
				 after init) */

void dyn_free();              /* destroy all trees and give up memory.
				 must not call anything after this */

/* Karger's algorithm wants something slightly different from find_bottleneck*/
dyn_item * dyn_find_min(dyn_item * from);
                                        /* Find the minimum cost node on the 
					   path from 'from' to its root */

/***** end of changes for K */

void dyn_make_tree(dyn_item * item, DOUBLE value);
...
dyn_item * din_find_father(dyn_item * item);
